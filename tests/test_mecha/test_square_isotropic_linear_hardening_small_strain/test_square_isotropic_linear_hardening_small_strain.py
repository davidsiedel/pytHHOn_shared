...
class TestMecha(TestCase):
    def test_square_traction_x_elsaticity(self):
        ...
        # time_steps = np.linspace(0.0, 0.008, 300, dtype=real)
        # time_steps = np.linspace(0.0, 0.01, 11, dtype=real)
        iterations = 12
        print(time_steps)
        ...
        # stabilization_parameter = 70.0e5 * parameters["YoungModulus"] / (1.0 + parameters["PoissonRatio"])
        # stabilization_parameter = 1.0 * parameters["YoungModulus"] / (1.0 + parameters["PoissonRatio"])
        stabilization_parameter = 1.e0 * parameters["YoungModulus"] / (1.0 + parameters["PoissonRatio"])
        ...
        mtest_file_path = (
            "mtest/traction_isotropic_linear_hardening_small_strain.res"
        )
        hho_file_path = "../../../res"
        ...
        plot_data_2(mtest_file_path, hho_file_path, len(time_steps), 1, 5, 4, 8)
        plot_data_2(mtest_file_path, hho_file_path, len(time_steps), 1, 6, 4, 9)
        plot_data_2(mtest_file_path, hho_file_path, len(time_steps), 1, 6, 4, 10)
        plot_data_2(mtest_file_path, hho_file_path, len(time_steps), 1, 7, 4, 11)
